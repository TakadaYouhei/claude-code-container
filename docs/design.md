...
- Podman 利用時は `docker-compose.podman.yml` を `-f docker-compose.yml -f docker-compose.podman.yml`
  で重ねて適用し、ボリュームマウントに `:Z` ラベルを付与する等の差分のみを吸収する。
- プロジェクトを複数並行稼働させる場合は `-p <project-name>` を指定し、ボリューム名の衝突を防ぐ
  （5章参照）。

対応要件: 4.4, 4.5, 4.8

## 5. 環境チェック設計（scripts/check-env.sh）

### 5.1 チェック項目と判定ロジック

| # | 項目 | 判定方法 | NG時の出力例 |
| --- | --- | --- | --- |
| 1 | コンテナエンジンの有無・バージョン | `docker --version` / `podman --version` の実行可否とバージョン比較 | インストール手順 URL を提示 |
| 2 | compose ツールの有無 | `docker compose version` / `podman-compose --version` の実行可否 | 導入コマンド例を提示 |
| 3 | 必須コマンド | `command -v git` 等の存在確認 | パッケージマネージャ別インストールコマンドを提示 |
| 4 | ディスク空き容量 | `df` で作業ディレクトリのマウント先空き容量を取得し閾値と比較 | 必要空き容量と現状値を提示 |
| 5 | メモリ | `/proc/meminfo` 等から総メモリ量を取得し閾値と比較 | 必要メモリ量と現状値を提示 |
| 6 | ネットワーク到達性 | `curl -sSf https://api.anthropic.com` 等への到達確認 | プロキシ設定・ファイアウォール確認を促す |
...
## 8. コンテナエンジン差異吸収設計（Docker / Podman）
...
| compose 実行コマンド | `docker compose` | `podman-compose` または `podman compose` | 起動スクリプト側でコマンドの存在確認をして自動選択 |
...
起動スクリプトがこの値を見て使用する compose コマンド・override ファイルを切り替える。
...
- 認証情報を含むボリューム（`claude-config`, `dotfiles` 内のSSH鍵）は、ホスト側で
  パーミッション 600 / ディレクトリ 700 を徹底する。運用手順は取扱説明書に明記する。
- 自動承認モードは非root実行・作業ブランチの分離（10章）と組み合わせて運用し、
...
- ホスト・コンテナの再起動後は `docker compose up -d` / `podman-compose up -d` のみで
  復旧できる（5章のボリューム設計により状態が保持されるため、追加の復旧手順を要しない）。

対応要件: 5.2, 5.3

## 13. 要件定義書「今後の検討事項」への対応方針（本設計での暫定決定）

| 検討事項 | 本設計での方針 |
| --- | --- |
| commit粒度 | 1指示（1ラウンド）につき1コミットを既定とする（10.3） |
...
| セッション開始・終了の検知方式 | CLI提供のフックを優先、無い場合は起動ラッパー（`session-branch.sh`）で代替（10.1） |
